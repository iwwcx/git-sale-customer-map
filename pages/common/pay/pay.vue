<template>
  ...
  <view v-else class="pay-page">
    ...
    <view class="hero">
      ...
      <view class="hero-title">{{ isOfficialMember ? '探客工兵会员续费' : '欢迎成为探客工兵尊享会员' }}</view>
      <view class="hero-sub">
        <block v-if="isOfficialMember && expireTime"><text style="color: white;">当前会员到期时间：<text style="color: orange;font-weight: bold;font-size: 28rpx;">{{ expireTime }}</text></text></block>
        <block v-else>让销售更轻松，目标更精准</block>
      </view>
      ...
      <view class="pay-method">
        <view class="method-left">
          <view class="method-icon bank-method-icon">
            <text class="bank-method-text">CMB</text>
          </view>
...
export default {
  ...
  data() {
    return {
      ...
      isOfficialMember: false, // 是否是正式会员
      expireTime: '', // 会员到期时间
      ...
      benefits: [
        '无限制查看公司及潜在客户联系人详情',
        '一键直联服务商，随时随地的信息技术支持',
        '任意搜索关注地区市场需求信息，销售更精准',
        '地图可视化预览身边客户资源，快速查目标客户'
      ]
    }
  },
  computed: {
    ...
    showUnderMaintenance() {
      const now = new Date()
      const deadline = new Date(2026, 5, 15, 12, 0, 0)
